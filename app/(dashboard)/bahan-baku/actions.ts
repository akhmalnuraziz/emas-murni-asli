...
async function uploadSignaturePlb(supabase: SupabaseClient, dataUrl: string, prefix: string): Promise<string | null> {
  try {
    ...
    const { error } = await supabase.storage.from('fotos').upload(path, buffer, { contentType: `image/${ext === 'jpg' ? 'jpeg' : ext}`, upsert: true })
...
export async function editPeleburanSerah(id: number, formData: FormData) {
  ...
  const { data: plb } = await supabase.from('peleburan')
    .select('kode, dikasih_gram, batch_kode, diterima_gram').eq('id', id).single()
  if (!plb) return { error: 'Peleburan tidak ditemukan' }
  ...
  const { error } = await supabase.from('peleburan').update({
    ...
    ...(timId !== null ? { tim_id: timId, tim_nama: timNama } : {}),
...
export async function editPeleburanTerima(id: number, formData: FormData) {
  ...
  const { error } = await supabase.from('peleburan').update({
    diterima_gram: diterima, tanggal_diterima: tanggalTerima, jam_selesai: jamSelesai,
    operator_diterima: terimaAdmin,
    keterangan_diterima: keteranganTrm,
    foto_diterima: fotoDtUrls, status: 'selesai',
    ...(terimaTimId ? { tim_id: terimaTimId, tim_nama: terimaTimNama } : {}),
    ...(terimaAdmin ? { admin_input: terimaAdmin } : {}),
  }).eq('id', id)
