  ...
  mobileOpen && 'translate-x-0',
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 bg-gradient-to-br from-violet-600 to-violet-500 rounded-lg flex items-center justify-center flex-shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
          {NAV_ITEMS.filter(item => hasAccess(item.module as any)).map(item => {
            ...
            return (
              ...
              >
                <Icon size={16} className={active ? 'text-violet-600' : 'text-slate-400 group-hover:text-slate-500'} />
                <span className="flex-1 truncate">{item.label}</span>
                {active && <ChevronRight size={13} className="text-violet-400 flex-shrink-0" />}
              </Link>
            )
          })}
        </nav>
        ...
        <div className="px-3 pb-4 pt-2 border-t border-slate-100">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-50">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-400 capitalize truncate">{profile?.role?.replace('_', ' ')}</p>
            </div>
            <button
              onClick={signOut}
