...
const NAV_GROUPS = [
  {
    ...
    items: [
      ...
      { href: '/laporan/efisiensi',   label: 'Efisiensi & Losses',   icon: TrendingDown,    module: 'laporan-efisiensi'  },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/stok-cabang',           label: 'Stok & PO Cabang',     icon: MapPin,         module: 'stok-cabang'          },
    ...
    return (
      <Link
        ...
        className={cn(
          ...
          active
            ? 'bg-violet-600/10 text-violet-700 font-semibold shadow-[inset_3px_0_0_theme(colors.violet.500)] backdrop-blur-sm'
  ...
  return (
    <>
      ...
      <aside className={cn(
        ...
        {/* ── Logo ── */}
        <div className="flex items-center justify-between px-4 h-[64px] flex-shrink-0 border-b border-white/40">
        ...
        {/* ── Bottom nav ── */}
        <div className="px-2 py-2 border-t border-white/40 space-y-px">
          {BOTTOM_NAV.filter(item => canShow(item.module as any)).map(item => renderItem(item))}
        </div>

        {/* ── User ── */}
        <div className="px-3 pb-3 pt-2 border-t border-white/40">
          <div className="flex items-center gap-2.5 px-2.5 py-2 rounded-xl glass border border-white/50">
      ...
      {confirmLogout && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm glass-modal rounded-2xl shadow-2xl p-5 space-y-4">
