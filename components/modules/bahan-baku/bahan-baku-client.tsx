      ...
      <div className="w-full sm:max-w-md bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          <form id="ringkas-form" onSubmit={e=>{e.preventDefault();onSubmit(new FormData(e.currentTarget))}} className="space-y-4">
            <div>
              <label className="block text-[11px] font-medium text-slate-500 mb-1.5">Kode Batch *</label>
              <input name="kode" required placeholder="mis. B-001 atau BATCH-001" className={`${inp} font-mono font-semibold`} autoFocus/>
            </div>
            <div>
              <label className="block text-[11px] font-medium text-slate-500 mb-1.5">Tanggal Masuk *</label>
              <input name="tanggal" type="date" required defaultValue={new Date().toISOString().split('T')[0]} className={inp}/>
            </div>
            <div>
              <label className="block text-[11px] font-medium text-slate-500 mb-1.5">Berat (gr) *</label>
        ...
        <form id="batch-form" onSubmit={submit}className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <label className="block text-[11px] font-medium text-slate-500">Biaya tambahan</label>
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-50 border border-slate-200">
              <div className="flex-1 space-y-1.5">
                ...
                <div className="flex justify-between text-[12px] border-t border-slate-200 pt-1.5">
                  ...
                  <span className="font-semibold text-violet-700">{formatRupiah(totalHpp)}</span>
                </div>
                {hppGr>0&&<div className="flex justify-between text-[12px]">
                  <span className="font-semibold text-slate-500">HPP / gram</span>
                  <span className="font-semibold text-violet-700">{formatRupiah(hppGr)}/gr</span>
                </div>}
              </div>
            </div>
          )}
    ...
    <div className="space-y-5 pb-8">
        ...
        <div className="space-y-3">
          {filtered.length===0?(
            ...
            return(
              <div key={batch.id} className="bg-white border border-slate-200/80 rounded-xl shadow-sm overflow-hidden transition-all">
                {/* Card header */}
                <div className="flex items-center gap-3 px-4 pt-4 pb-3">
                  <div className="w-9 h-9 rounded-lg bg-violet-50 flex items-center justify-center flex-shrink-0 font-semibold text-[13px] text-violet-600">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[12px] text-slate-400 mt-0.5 font-medium truncate">{batch.kode} · {supplierLabel}</p>
                    <p className="text-[11px] text-slate-400">Datang: {formatDate(batch.tanggal)}</p>
                  </div>
                ...
                {isExp&&(
                  <div className="px-5 pb-5 border-t border-slate-200 space-y-4 bg-violet-50/20">
                      ...
                      return (
                        <div className="rounded-lg overflow-hidden border border-red-200">
                          <div className="px-3 py-2 bg-red-50 border-b border-red-100">
                            <p className="text-[11px] font-semibold text-red-600">⚠ Selisih timbangan — TTD tersimpan</p>
                          </div>
                    ...
                    {(()=>{
                      ...
                      return (
                        <div>
                          <p className="text-[11px] font-medium text-slate-500 mb-2.5 px-1">Rekonsiliasi detail</p>
                            ...
                            <div className="rounded-lg px-3.5 py-3 col-span-2 sm:col-span-3 bg-white border border-slate-200">
                              <div className="flex items-center justify-between mb-1.5 flex-wrap gap-2">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <p className="text-[10px] font-medium text-slate-500">Sisa fisik</p>
                                  {selisihSisaFisik!=null&&(
                                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${selisihSisaFisik>0?'bg-blue-50 text-blue-600':selisihSisaFisik<0?'bg-red-50 text-red-500':'bg-green-50 text-green-600'}`}>
                                ...
                                {status==='aktif'&&!isEditSF&&(
                                  <div className="flex items-center gap-1.5">
                                    <button onClick={()=>{setEditingSF(batch.id);setSfInput(p=>({...p,[batch.id]:String(sisaFisik??'')}));setSfExisting(p=>({...p,[batch.id]:[...fotoSF]}))}}
                                      className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-semibold text-violet-600 rounded-xl bg-white border border-violet-200">
                                      <Edit2 size={10}/>{sisaFisik!=null?'Edit':'Input'}
                                    </button>
                                    {sisaFisik!=null&&(
                                      <button onClick={()=>handleHapusSisaFisik(batch)}
                                        className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-semibold text-red-500 rounded-xl bg-white border border-red-200">
                              ...
                              {!isEditSF&&(
                                <p className="text-[14px] font-semibold text-green-600">{sisaFisik!=null?formatGram(sisaFisik):'—'}</p>
                              )}
                              {isEditSF&&(
                                <div className="space-y-2 mt-1">
                                  <div className="flex gap-2">
                                    ...
                                    <button onClick={()=>handleSisaFisik(batch,sisaSeharusnya,toleransiPeleburan)} disabled={sfUploading[batch.id]}
                                      className="px-3 h-8 text-[12px] font-semibold text-white rounded-lg bg-green-600 hover:bg-green-700 disabled:opacity-50 flex items-center gap-1 flex-shrink-0 transition-colors">
                                    ...
                                    <button onClick={()=>{setEditingSF(null);setSfFotos(p=>({...p,[batch.id]:[]}))} }
                                      className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0"><X size={12}/></button>
                                  </div>
                                  ...
                                  <input type="text" value={sfCatatan[batch.id]??''} onChange={e=>setSfCatatan(p=>({...p,[batch.id]:e.target.value}))}
                                    ...
                                    className="w-full px-3 py-2 text-[13px] rounded-xl border border-slate-200 bg-white focus:outline-none"/>
                    ...
                    <div className="rounded-lg overflow-hidden border border-slate-200">
                      <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border-b border-slate-200">
                        <p className="text-[11px] font-medium text-slate-500">🔥 Riwayat peleburan</p>
                        {status==='aktif'&&(
                          <button type="button" onClick={()=>setPeleburanModalBatch(batch.kode)}
                            className="flex items-center gap-1 px-2.5 h-7 rounded-lg text-[11px] font-semibold text-white bg-violet-600 hover:bg-violet-700 transition-colors">
                            + Buat Peleburan
                          </button>
                        )}
                      </div>
                      {(()=>{
                        ...
                        if(plbList.length===0) return (
                          <p className="text-[12px] text-slate-400 text-center py-4">Belum ada peleburan.</p>
                        )
                              ...
                              <div className="flex items-center gap-1 flex-shrink-0">
                                {plb.status!=='selesai'&&batch.status==='aktif'&&(
                                  <button type="button" onClick={()=>setSelesaiLeburItem(plb)}
                                    className="flex-shrink-0 px-2.5 h-7 rounded-lg text-[11px] font-semibold text-white bg-green-600 hover:bg-green-700 transition-colors">
                                    Selesai Lebur
                                  </button>
                                )}
                                {batch.status==='aktif'&&(
                                  hapusPlbId===plb.id ? (
                                    <div className="flex items-center gap-1 px-2 py-1 rounded-xl bg-red-50 border border-red-200">
                                      ...
                                      }} className="px-2 py-0.5 rounded-lg text-[10px] font-semibold bg-red-500 text-white">Ya</button>
                                      <button type="button" onClick={()=>setHapusPlbId(null)}
                                        className="px-2 py-0.5 rounded-lg text-[10px] font-semibold bg-slate-200 text-slate-600">Batal</button>
                                    </div>
                                  ) : (
                            ...
                            {plb.loss_approval&&(
                              <div className="mt-2 rounded-xl overflow-hidden border border-red-100">
                                <div className="px-3 py-2 flex items-center gap-2 bg-red-50">
                                  <span className="text-[10px] font-semibold text-red-600">⚠ TTD loss peleburan</span>
                                  <span className="text-[10px] text-red-400 ml-auto">Loss disetujui</span>
                                </div>
                                <div className="px-3 py-2.5 space-y-1.5">
                                  {plb.loss_approval.alasan&&(
                                    <p className="text-[12px] text-slate-600"><span className="font-semibold text-slate-700">Alasan:</span> {plb.loss_approval.alasan}</p>
                                  )}
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40">
      <div className="w-full sm:max-w-lg bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="rounded-lg overflow-hidden border border-slate-200">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-violet-50 border border-violet-100">
                ...
                <span className={`text-[13px] font-semibold ${totalDikasih>0?'text-violet-700':'text-slate-400'}`}>{(Math.round(totalDikasih*100)/100).toFixed(2).replace('.',',')} gr</span>
              </div>
            </div>
          </div>
          ...
          <div className="px-4 py-3 rounded-xl border border-dashed border-slate-200 text-center">
            <p className="text-[12px] text-slate-400">Bagian <span className="font-semibold text-slate-500">Diterima</span> diisi setelah proses lebur selesai</p>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40">
      <div className="w-full sm:max-w-lg bg-white rounded-xl border border-slate-200 shadow-xl overflow-hidden max-h-[92vh] flex flex-col">
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="rounded-lg overflow-hidden border border-slate-200">
            <div className="flex items-center gap-2 px-3 py-2 rounded-t-lg bg-slate-50 border-b border-slate-200">
              <span className="text-[11px] font-medium text-slate-500">📥 Diterima</span>
            </div>
            <div className="p-4 space-y-3">
              <div>
              ...
              <div>
                <label className="block text-[11px] font-medium text-slate-500 mb-1.5">Catatan Selesai Lebur</label>
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="rounded-lg overflow-hidden border border-slate-200">
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                <label className="block text-[11px] font-medium text-slate-500 mb-1.5">Tambah Foto Diterima</label>
