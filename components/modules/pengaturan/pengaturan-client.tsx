...
export default function PengaturanClient({
  ...
  const TABS = [
    { id: 'tim',   label: 'Master Tim',         icon: Users      },
    { id: 'admin', label: 'Master Admin Input',  icon: UserCheck  },
    { id: 'umum',  label: 'Pengaturan Umum',     icon: Settings2  },
  ] as const
      ...
      {editing ? (
        <div className="space-y-3">
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button onClick={() => { const fd=new FormData();fd.set('nama',nama);fd.set('warna',warna);start(async()=>{const r=await updateTim(tim.id,fd);if(r?.error){showToast('❌ '+r.error);return}showToast('✅ Tim diperbarui');onCancelEdit()}) }}
              disabled={isPending}
              className="px-5 py-2 rounded-xl text-sm font-bold text-white disabled:opacity-50"
              style={{ background: 'linear-gradient(135deg,#7F6DC6,#6857B1)' }}>Simpan</button>
          </div>
        ...
        <>
          ...
          <div className="mt-3 pt-3 border-t border-gray-100">
            <div className="flex flex-wrap gap-1.5 mb-2">
              {tim.anggota?.map((a: any) => (
                <span key={a.id} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium text-gray-600 bg-violet-50">
                  {a.nama}
                  <button onClick={() => start(async () => { const r=await deleteAnggota(a.id);if(r?.error){showToast('❌ '+r.error);return}showToast('✅ Anggota dihapus') })} className="text-gray-400 hover:text-red-500"><X size={11} /></button>
                </span>
              ))}
            ...
            <div className="flex gap-2">
              <input value={newAnggota} onChange={e => setNewAnggota(e.target.value)}
                ...
                onKeyDown={e => { if (e.key==='Enter') { start(async()=>{const r=await addAnggota(tim.id,newAnggota.trim());if(r?.error){showToast('❌ '+r.error);return}showToast('✅ Anggota ditambahkan');setNewAnggota('')}) } }}
              ...
              <button onClick={() => { if(!newAnggota.trim())return;start(async()=>{const r=await addAnggota(tim.id,newAnggota.trim());if(r?.error){showToast('❌ '+r.error);return}showToast('✅ Anggota ditambahkan');setNewAnggota('')}) }}
          ...
          {confirmDel && (
            <div className="mt-3 p-3 rounded-2xl flex items-center gap-3 bg-red-50 border border-red-100">
              ...
              <button onClick={() => start(async()=>{const r=await deleteTim(tim.id);if(r?.error){showToast('❌ '+r.error);return}showToast(r.softDeleted?'✅ Tim dinonaktifkan':'✅ Tim dihapus');setConfirmDel(false)})}
                disabled={isPending} className="px-3 py-1.5 rounded-lg text-xs font-bold text-white bg-red-500 disabled:opacity-50">Hapus</button>
            </div>
          )}
        </>
      )}
...
// ═══════════════════════════════════════════════════════════════════════════
// MASTER ADMIN INPUT (PRD Bagian 28 — terpisah dari Tim)
// ═══════════════════════════════════════════════════════════════════════════
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-400 mt-0.5">
          Daftar admin yang bisa dipilih saat mencatat serah-terima. Berbeda dari Tim (tim = yang mengerjakan, admin = yang input ke sistem).
        </p>
      </div>
      ...
      {list.length === 0 && (
        <div className="rounded-3xl py-14 text-center bg-white border border-slate-100">
          ...
          <p className="text-xs text-gray-300 mt-1">Dipakai sebagai dropdown "Admin Yang Input" saat mencatat produksi, packing, dll.</p>
        </div>
      )}
            ...
            {editId === a.id ? (
              <>
                <input value={editNama} onChange={e => setEditNama(e.target.value)}
                  autoFocus className="flex-1 h-9 px-3 bg-gray-50 rounded-xl text-sm border border-gray-200 focus:outline-none focus:ring-2 focus:ring-violet-200"
                  onKeyDown={e => { if (e.key==='Enter') { start(async()=>{const r=await updateAdminInput(a.id,editNama.trim());if(r?.error){showToast('❌ '+r.error);return}showToast('✅ Diperbarui');setEditId(null)}) } }} />
                <button onClick={() => { start(async()=>{const r=await updateAdminInput(a.id,editNama.trim());if(r?.error){showToast('❌ '+r.error);return}showToast('✅ Diperbarui');setEditId(null)}) }}
              ...
              <>
                ...
                {canManage && (
                  <>
                    <button onClick={() => start(async () => { await toggleAdminInputAktif(a.id, !a.aktif); showToast(a.aktif ? 'Dinonaktifkan' : 'Diaktifkan') })}
                    ...
                    {confirmDelId === a.id ? (
                      <>
                        <button onClick={() => start(async()=>{const r=await deleteAdminInput(a.id);if(r?.error){showToast('❌ '+r.error);return}showToast('✅ Dihapus');setConfirmDelId(null)})}
                          disabled={isPending} className="px-2 py-1 rounded-lg text-xs font-bold text-white bg-red-500 disabled:opacity-50">Hapus</button>
...
// ═══════════════════════════════════════════════════════════════════════════
// PENGATURAN UMUM — toleransi + ambang + KPI bobot
// ═══════════════════════════════════════════════════════════════════════════
function PengaturanUmumSection({ pengaturan, isPending, start, showToast, canManage }: any) {
  ...
  const ambangRows = [
    ['ambang_gain_wajar',        'Gain Wajar (max tanpa persetujuan)', '✨', 'Gain di bawah nilai ini dianggap wajar, tidak menurunkan KPI'],
    ['ambang_loss_kumulatif',    'Loss Kumulatif Waspada per Batch',  '⚠️', 'Jika total loss batch melebihi ini, sistem kirim notif owner'],
  ] as const
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Batas loss yang diizinkan (gram). Jika melebihi, sistem minta tanda tangan operator & admin.</p>
        </div>
        <div className="rounded-3xl p-4 space-y-3 bg-white border border-slate-100 shadow-sm">
          {toleransiRows.map(([key, label, icon]) => (
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Nilai batas untuk trigger notif & penentuan gain wajar.</p>
        </div>
        <div className="rounded-3xl p-4 space-y-4 bg-white border border-slate-100 shadow-sm">
          {ambangRows.map(([key, label, icon, desc]) => (
            <div key={key}>
